...
function getBirthdayObject(petInfo) {
    return new Date(petInfo.year, petInfo.month - 1, petInfo.day); // -1 because month is indexed at 0
}
...
function getProfilePhotoSrc(petInfo, isLocalHost) {
    console.log(petInfo);
    if (petInfo.selectPhoto === 'Custom') {
        return petInfo.photoUpload;
    }
    else {
        let url = `/assets/profile-photos/${petInfo.selectPhoto.toLowerCase().replaceAll(' ', '-')}.png`;
        if (!isLocalHost) url = chrome.runtime.getURL(url);
        return url;
    }
}
