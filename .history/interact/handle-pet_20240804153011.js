...
function handleEvents() {
    ...
    const whilePettingUpdateTime = 500;
    onMouseRelease(pet.imgEl, whilePetting, completePetting, 500); // Pet action if click and release on pet
}

function whilePetting() {
    if (!interactContainer) return;
    const cursor = getCursor();
    createParticles('heart', cursor.pos, interactContainer);
}
