import { randomFromRange } from '/content/utils/general-utils.js';
...
function getBiteRadius(numBites) {
    ...
    return randomFromRange(minRad, maxRad); // Return radius between minRad and maxRad
}
...
function countPixelsLeft(canvas, ctx) { // Counts non-transparent pixels
    ...
    for (let i = 0; i < buffer.length; i++) {
        const alpha = (buffer[i] >> 24) & 0xFF; // Extract alpha channel
        if (alpha !== 0) c++;
    }
