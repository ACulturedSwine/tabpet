...
/* Position functions
    ...
    Note that isElAtPos, isTouching, and isPosInsideEl have similar code, but are semantically different.
*/
...
function isElAtPos(el, pos) { // Code is similar to isPosInsideEl, but semantically different. Returns info on where pos is in relation to el.
    if (!pos.x || !pos.y) throw new Error('Position must have x and y defined', pos);
    const dims = el.getBoundingClientRect();
    const toLeft = pos.x < dims.left;
    const toRight = pos.x > dims.right;
    const above = pos.y > dims.top;
    const below = pos.y < dims.bottom;
