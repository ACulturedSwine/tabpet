...
function isElAtPos(el, pos) { // Returns info on where pos is in relation to el.
    ...
    const dims = el.getBoundingClientRect();
    return collisionLogic(pos.x < dims.left, pos.x > dims.right, pos.y > dims.top, dims.bottom);
}
...
function isAt(a, b) { // Given two elements/points a and b, returns information on if a and b are touching and where b is in relation to a. 
