...
class Pet {
    ...
    #move(xDis, yDis) { // Moves if hitbox not out-of-bounds. Returns true if can move, false if can't.
        let inBounds = true;
        if (inBounds) {
            if (xDis * this.flip < 0) { // If xDis and scale are different signs, flip pet.
                this.flip *= -1;
                this.flipEl.style.transform 
            }
    ...
    #createEls(params) { // Create elements of pet, return outermost element
        ...
        this.flip = 1; // Either 1 or -1. 1 means facing right, -1 means facing left.
