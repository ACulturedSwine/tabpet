...
class Emote {
    ...
    randomEmoji() {
        console.log(this.emojiCategories, this.emojiIndex);
        return randomFromArray(randomFromArray(this.emojiCategories)); // Gets random emoji from random emoji category;
    }

    async emote(params={path: '/assets/emote/speech/1.png', content: '', lingerTime: 2}) { // Time in seconds
        // Set emote
        this.contentEl.textContent = params.content;
        this.imgEl.src = chrome.runtime.getURL(params.path);

        // Show for an amount of time and then hide
        this.showEmote();
        await wait(params.lingerTime, 'seconds');
        this.hideEmote();
    }
