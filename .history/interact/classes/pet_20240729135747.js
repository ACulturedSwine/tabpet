...
class Pet {
    ...
    #move(xDis, yDis) { // Moves if hitbox not out-of-bounds. Returns true if can move, false if can't.
        let inBounds = true;
        if (inBounds) {
            if (xDis * this.scale < 0) this.scale *-1; // If xDis and scale are different signs, flip pet
    ...
    #createEls(params) { // Create elements of pet, return outermost element
        this.scale = params.scale || 0.3; // Value resizes pet. + means facing right, - means facing left.
