...
class Emote {
    constructor() {
        this.createEl();
        this.createEmojis();
        this.emote({content: this.randomEmojiFromAnyCategory('food')});
    }
    ...
    randomEmote() {
        ...
        let content = '';
        if (name === 'speech') content = this.randomEmojiFromRandomizableCategory();
        // Set emote
    ...
    randomEmoji(type='normal', setCategory) {
        let category = setCategory || this.emojiIndex[]   
    }

    randomEmojiFromRandomizableCategory() { // Gets random emoji from randomizable emojis (in emojiIndex) 
    ...
    randomEmojiFromAnyCategory(category) { // Gets random emoji from any category
        ...
        // Search all emoji indexes
        if (this.emojiIndex.hasOwnProperty(category)) emojis = this.emojiIndex[category];
        else if (this.specialIndex.hasOwnProperty(category)) emojis = this.specialIndex[category];
        else throw new Error(`Category ${category} not found`);
    ...
    createEmojis() {
        ...
        for (let category in normalIndex) {
        ...
        this.emojiIndex = {'normal': {index: normalIndex, categories: }, 'special': specialIndex};
    }
}
