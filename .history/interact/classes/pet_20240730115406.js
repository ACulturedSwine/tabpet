...
class Pet {
    ...
    /* Macro behaviors */
    findFood(food) {
        if (this.findingFood || !(food instanceof Interactable) || food.type !== 'food') return;
        ...
        const touchingFood = isTouching(this.hitboxEl, food.el);
        if (touchingFood) this.eat(foodObj);
    }
    ...
    /* Interaction */
    touching(el) {
        return isTouching(this.hitboxEl, );
    }
    ...
    #moveToEl(el) {
        ...
        return new Promise((resolve)=> {
            const loopID = setInterval(function() {
                let found = isTouching(this.hitboxEl, )
            }, updateTime);
        })
    }
