...
class Pet {
    ...
    isTouchingSearchBar(searchBar) {
        return this.isThisColliding(searchBar);
    }

    handlePseudoTypeSearch() {
        const searchBar = document.querySelector('#pet-profile-icon');
        if (this.isTouchingSearchBar(searchBar)) console.log('touching search bar');
    }

    pet() {
        this.emote.heartEmote();
        createPopupMsg({content: `Petted ${this.petInfo.firstName}`, waitTime: 2000});
    }
