...
class Pet {
    ...
    /* Activity levels */
    initActiveLvlSettings(initLvl='normal') {
        this.activeIndex = {
            'normal': {breathTime: 2, anim: 'squish', icon: '😎', alerts: ['is up and ready!']},
            'relaxed': {breathTime: 4, anim: 'smoosh', icon: '🌿', alerts: ['is feeling cozy!']}
        };
        this.setActiveLvl(initLvl, false); // Update active level and don't send a popup message about it
    }

    displayActiveLvl() {
        console.log('active lvl', this.activeLvl, 'breath time', this.breathTime);
        const lvl = this.activeIndex[this.activeLvl];
        this.animate(lvl.anim, lvl.breathTime);
    }

    setActiveLvl(lvl, sendAlert=true) {
        this.activeLvl = lvl;
        const info = this.activeIndex[lvl];
        this.breathTime = info.breathTime;
        if (sendAlert) {
            const alert = this.petInfo.firstName + ' ' + randomFromArray(info.alerts);
    ...
    relax(change=0.2) {
        ...
        if (this.activeLvl === 'relaxed') return;

        // Else, update breath time and animate active level
        const maxBreathTime = this.activeIndex['relaxed'].breathTime;
        this.breathTime = Math.min(this.breathTime + change, maxBreathTime);
        if (this.breathTime >= maxBreathTime) this.setActiveLvl('relaxed');
        this.displayActiveLvl();
    }
