...
class Emote {
    constructor() {
        this.createEl();
        this.createEmojis();
        this.emote({content: this.randomEmoji('special', 'food')});
    }
    ...
    randomEmoji(type='normal', setCategory) {
        ...
        if (!index) throw new Error(`Index type ${index} not found in emojis index`);
        let category = setCategory || randomKeyFromObject(index);
        let emojis = index[category];
        if (!emojis) throw new Error(`Category ${category} not found in index ${index}`);
