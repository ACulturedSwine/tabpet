...
class Pet {
    ...
    /* Activity levels */
    initActiveLvlSettings(initLvl) {
        this.activeLvls = {
            'normal': {breathTime: 2, anim: 'squish'},
            'relaxed': {breathTime: 4, anim: 'smoosh'}
        };
        this.activeLvl = initLvl;
        this.breathTime = this.activeLvls[initLvl].breathTime;
    }

    animateActiveLvl() {
        console.log('active lvl', this.activeLvl);
        const lvl = this.activeLvls[this.activeLvl];
    ...
    relax(change=0.2) {
        ...
        const maxBreathTime = this.activeLvls['relaxed'].breathTime;
        this.breathTime = Math.min(this.breathTime + change, maxBreathTime);
        if (this.breathTime >= maxBreathTime) this.activeLvl = 'relaxed';
        this.animateActiveLvl();
    }
