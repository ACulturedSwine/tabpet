...
import { isTouching } from '/interact/utils/interact-utils.js';
...
class Pet {
    ...
    async handleFood(food) {
        if (this.handlingFood || !this.isFood(food)) return;
    ...
    eat(food) {
        if (!this.isFood(food)) return;

        let finished = false;
        let loopID = setInterval(function() {
            finished = food.eat();
            if (finished) clearInterval(loopID);
        }, 1000);
    ...
    isFood(food) {
        return food instanceof Interactable && food.type === 'food';
    }
    ...
    #moveToEl(el) {
        ...
        return new Promise((resolve)=> {
            const loopID = setInterval(function() {
                const collisionInfo = this.touching(food.el);
                if (collisionInfo.touching) {
                    clearInterval();
                    resolve(true);
                }
                else {
                    if (collisionInfo.toLeft) this.#left();
                    else if (collisionInfo.toRight) this.#right();
                    if (collisionInfo.above) this.#up();
                    if (collisionInfo.below) this.#down();
                }
            }, updateTime);
        })
    }
