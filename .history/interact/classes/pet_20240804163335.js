...
class Pet {
    ...
    setActiveLvl(activeLvl) {
        if (!this.activeLvls.hasOwnProperty(activeLvl)) throw new Error(`Active level ${activeLvl} not implemented`);
        this.activeLvl = activeLvl;
        const activeLvlInfo = this.activeLvls[activeLvl];
        this.animate(activeLvlInfo.anim, activeLvl);
    }
    ...
    relax(change=0.2) {
        if (this.activeLvl = 'relaxed') return; // Max level of relaxation
