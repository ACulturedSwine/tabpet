...
import { isFood } from '/interact/utils/interact-utils.js';
import { isColliding } from '/interact/utils/collision.js';

class Pet {
    constructor(createParams, personalInfo) {
        this.type = createParams.type;
        this.subtype = createParams.subtype;
    ...
    executeEvent(event) {
        ...
        return new Promise(async (resolve)=> {
            switch(name) {
                case 'eat':
                    await pet.handleFood(params.food);
                    break;
            }
            resolve(true);
        })
    ...
    handleFood(food) {
        ...
        return new Promise(async (resolve)=> {
            ...
            if (movedToFood) await this.eat(food);
            resolve(true);
        })
    }
    ...
    /* Interaction */
    isPetColliding(el) {
        return isColliding(this.hitboxEl, el);
    }
    ...
    moveTo(place) { // Place is either HTML Element or position (x, y)
        ...
        return new Promise((resolve)=> {
            const loopID = setInterval(function() {
                ...
                try {
                    collisionInfo = isColliding(pet.hitboxEl, place);
