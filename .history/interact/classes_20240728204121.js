import * as Food from '/interact/food-utils.js';
...
class Interactable {
    ...
    #initFood() {
        ...
        this.numBites = 0;
        this.startingPixels = Food.countPixelsLeft(this.canvas, this.ctx); // Non-transparent pixels

        console.log('img loaded', this.imgLoaded);

        console.log('starting pixels from func', this.startingPixels);
        console.log('canvas width x height', this.canvas.width * this.canvas.height);
    }

    eat() { // Creates bite and returns true if can eat more, returns false if can't
        if (this.type !== 'food') return;
        
        const canEat = Food.canEat(this.startingPixels, this.canvas, this.ctx);

        console.log('img loaded here', this.imgLoaded);
