...
import { resetCursor, setCursor, isInside, waitUntilImgLoaded } from '/interact/utils/interact-utils.js';
...
function handleInteract() {
    document.addEventListener('remove-interactable', (e)=> {
        console.log('received remove request', e.detail);
...
async function createInteractable(pasteType, src, pos, interactContainer) {
    ...
    if (newInteractable.hasImg && !newInteractable.imgLoaded) await waitUntilImgLoaded(newInteractable.img);
...
function removeInteractable(pasteID) {
    ...
    if (!obj || !(obj instanceof Interactable)) {
        console.error(`Requested pasteID has no object associated with it or isn't a `, obj);
        return;
    }
