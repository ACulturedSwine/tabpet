class Interactable {
    ...
    #createFoodEl(src) {
        ...
        img.addEventListener('load', ()=> {
            ...
            if (this.size.height) {
                hRatio = this.size.height / h;
            }
            else if (wRatio !== 1) { // Height is auto and set width is not image width
                hRatio = wRatio;
            }
            ...
            canvasEl.width = 100;
            canvasEl.height = 100;
            ctx.drawImage(img, 0, 0, 100, 100); // draws image with top left corner coords (0, 0) and canvas <width x height>
        });
