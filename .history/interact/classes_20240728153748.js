...
class Interactable {
    constructor(type, src, position, containerEl) {
        ...
        if (type === 'food') this.initEl = this.#initFood;

        this.createEl(src, position);
        this.#initPos(position.x, position.y);
    }

    createEl(src, position) { /* Initialize this.el. Public so function can be overwritten in subclasses of Interactable. */
        this.el = this.#createImgEl(src);
    }

    /* Food functions */ 

    #createFood(src, position) {
