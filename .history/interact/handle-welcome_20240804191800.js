...
function handleLastOpened() { // Update last opened and return if opened today
    return new Promise(async (resolve)=> {
        ...
        chrome.storage.sync.set({'lastOpened': today.now() }); // Save Date as Date.now (number) since Chrome stores only JSON-compatible types
        console.log(res);
        return;
        // Never opened before
        if (!res.lastOpened) resolve(false);
        // Check if last opened was today
        let lastOpened = Date.parse(res.lastOpened);
