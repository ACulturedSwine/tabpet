...
let pettingTime = 0; // In milliseconds, measures 
const pettingUpdateTime = 500; // In milliseconds
...
function whilePettingUpdate() {
    ...
    if (isColliding(cursor.pos, pet.hitboxEl).yes) createParticles('heart', cursor.pos, interactContainer);
}

function whilePettingInterval() {
    pettingTime = 0;
    const id = setInterval(whilePettingUpdate, 500);
    pettingTime
    return id;
}
