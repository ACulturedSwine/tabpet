class Interactable {
    ...
    #createBiteRadius(numBites) {
        ...
        const incrementBy = 10; // How much to increment
        const totalIncrement =  * increment;

        if (numBites % incrementEveryNBites == 0) {
            minRad + numBites/3 * 10;
    ...
    #canEat() {
        ...
        const canEat = pixelsLeft > this.startingPixels * minPixelPercentage;
        if (!canEat && pixelsLeft > 0) this.#clearCanvas(this.canvas, this.ctx); // Clear remaining canvas if there are still pixels
        return canEat;
    }
