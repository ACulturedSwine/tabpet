...
import { isSpecialDay, getTimeOfDay } from '/utils/date.js';
...
async function welcome(pet) {
    ...
    const lastOpened = await handleLastOpened();
    if (specialDay) {
...
function handleLastOpened() { // Return last opened date and update last opened
    return new Promise((resolve)=> {
        const lastOpened = chrome.storage.sync.get('lastOpened');
