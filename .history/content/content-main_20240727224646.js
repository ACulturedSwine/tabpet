...
function isCursorInside(el) { // Returns cursor position relative to element if inside element
    return new Promise(async (resolve)=> {
        ...
        if (pos.x >= elDimensions.left && pos.x <= elDimensions.right && pos.y >= elDimensions.top && pos.y <= elDimensions.bottom) { // is inside
            relativePos = {x: pos.x - elDimensions.left, y: pos.y - elDimensions.top};
        }
        resolve(relativePos);
    })
} 