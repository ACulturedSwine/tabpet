...
function createPetProfileWindow(petInfo) {
    ...
    // Create information
    const { heading, basicInfo, secondaryInfo } = createPetProfileInformation(petInfo);

    // Append information to window
    appendChildren(window, [heading, basicInfo, secondaryInfo]);
...
function createPetProfileInformation(petInfo) {
    // Heading
    const heading = createIDElement('div', 'pet-profile-heading', 'Information');

    // Primary information (in same line)
    const primaryInfo = createIDElement('div', 'pet-profile-primary');
