...
function createPetProfileWindow(petInfo) {
    ...
    // Create information
    const { header, basicInfo, secondaryInfo } = createPetProfileInformation(petInfo);

    // Append information to window
    appendChildren(window, [header, basicInfo, secondaryInfo]);
...
function createPetProfileInformation(petInfo) {
    ...
    // Return to handle
    return { header, primaryInfo, secondaryInfo };
}
