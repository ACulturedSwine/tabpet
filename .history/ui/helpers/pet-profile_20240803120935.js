import { createIcon, isIcon } from '/utils/icon.js';
...
function createPetProfileIcon(petInfo) {
    let src = getProfilePhotoSrc(petInfo);
    if (!src) src = chrome.runtime.getURL('/icon/32.png');
    const icon = createIcon(src, 'src');
...
function togglePetProfileWindow(e) {
    ...
    if (!isOpen && !isIcon(e.target, 'pet-profile-icon')) return;
