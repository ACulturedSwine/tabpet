...
function pseudoTypeSearch() {
    ...
    if (isOverflowing(customSearchText)) { // Clear text if overflow
        ...
        .textContent += iterateText(customSearchText);
    }
    else if (chance(1)) { // Random chance to get text
        text = texts[2]// randomFromArray(texts);
        customSearchText.textContent = '';
        customSearchText.textContent += iterateText(customSearchText);
    }
    else if (customSearchText.textContent.length >= minLengthToClear && chance(0.1)) { // For random strings, have possibility of clearing text
        customSearchText.textContent = '';
    }
