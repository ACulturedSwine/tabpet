...
class FormManager {
    constructor() {
        this.info = {}; // Dictionary for invalid inputs info, indexed by inputName
    ...
    */
    ...
    async saveInput(questionEl) {
        const inputName = questionEl.getAttribute('inputName');
        ...
        this.savedInputs[inputName] = value;
    ...
    #validate(el) { // Add if invalid, remove if valid and currently in invalid inputs
        ...
        const inputName = el.getAttribute('inputName');

        if (msg) { // Hard invalid, make input red and add to invalid inputs
            this.#add(inputName, el, msg);
            return true;
        }
            ...
            if (this.#has(inputName)) this.#remove(inputName);
