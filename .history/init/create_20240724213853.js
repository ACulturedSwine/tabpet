...
function handleNext(pages) {
    ...
    nextButton.addEventListener('click', ()=> {
        ...
        const inputs = currentPageEl.querySelectorAll('input, select');
        if (!inputs) {
...
function displayNewPage(pages, pageNum) {
    ...
    // clear warning and last invalid input (may be on new page)
    clearWarning();
    clearLastInvalidInput();
...
function clearLastInvalidInput() {
    if (lastInvalidInput && !isInvalid(lastInvalidInput)) lastInvalidInput.setAttribute('aria-invalid', false);
}