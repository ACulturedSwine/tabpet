...
function handleValidatePattern() {
    ...
    inputs.forEach((input)=> {
        ...
        input.addEventListener('input', function() { // for hard invalids (not blank), prevent user from changing input to invalid input
            const isValid = !invalidInputs.addIfInvalid(this);
            if (isValid) lastValidValue = this.value;
            /* 
            if (setWarningIfInvalid(input)) { // for invalid, prevent user from changing input to invalid input
                
                this.value = lastValidValue;
            } 
            else { // valid, update last valid value
...
function handleNext(pages) {
    ...
    nextButton.addEventListener('click', ()=> {
        ...
        if (res) { // invalid input
            ...
            if (lastInvalidInput && !isBlank(lastInvalidInput)) clearLastInvalidInput();
