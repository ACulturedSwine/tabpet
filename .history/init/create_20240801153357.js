...
import { formatFullName, formatBirthday, getProfilePhotoSrc } from '/utils/pet-info.js';
...
function displayNewPage(pages, pageNum) {
    ...
    if (pageNum === pages.length) showSavedInputs(currentPageEl);
...
function showSavedInputs(currentPageEl) { // Shows saved inputs in session.
    console.log(currentPageEl);

    currentPageEl.getElementById('chosen-photo').src = getProfilePhotoSrc(savedInputs);
    currentPageEl.getElementById('chosen-name').textContent = `Name: ${formatFullName(savedInputs)}`;
    currentPageEl.getElementById('chosen-birthday').textContent = `Born on ${formatBirthday(savedInputs)}/`;
    currentPageEl.getElementById('chosen-gender').textContent = savedInputs.gender;
    currentPageEl.getElementById('chosen-color').textContent = savedInputs.color;
    currentPageEl.getElementById('chosen-gender').textContent = savedInputs.gender;
}
