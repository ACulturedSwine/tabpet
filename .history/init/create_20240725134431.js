...
function handleNext(pages) {
    ...
    nextButton.addEventListener('click', ()=> {
        ...
        const isPageComplete = formManager.checkCurrentQuestions(currentPageEl) === null; // Returns null if no incomplete required questions.
        ...
        /*
        const inputs = getPageQuestions();
        if (!inputs) {
            console.error('No inputs!');
        }
        const res = saveAnswersIfValid(inputs);
