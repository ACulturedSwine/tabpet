...
import { getProcessedPersonalInfo } from '/utils/process-saved-inputs.js';
...
function showSavedInputs(currentPageEl) { // Shows saved inputs in session.
    const processed = getProcessedPersonalInfo(savedInputs);

    currentPageEl.getElementById('chosen-name').textContent = `Name: ${savedInputs.name}`;
    const birthday = `${savedInputs.month}/${savedInputs.day}/${savedInputs.year}`;
    currentPageEl.getElementById('chosen-name').textContent = `Born on ${birthday}/`;
