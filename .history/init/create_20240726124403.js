...
function handlePhotoUpload() {
    ...
    selectPhoto.addEventListener('change', function() {
        if (this.value === 'Custom') { // Allow user to select 'Custom' multiple times
            photoUpload.click(); // Only change last selected value to 'Custom' if user uploads file
        }
        else if (this.value !== lastSelectValue) {
