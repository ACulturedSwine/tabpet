...
function handlePaging() {
    const pages = [
        ...
        new FormPage('profilephoto', 'Profile photo', 'Upload a photo')
    ];
...
function handlePhotoUpload() {
    ...
    photoUpload.addEventListener('input', (e)=> {
        if (e.target.files && e.target.files[0]) {
            photo.setAttribute('src', window.URL.createObjectURL(uploader.files[0]));
        }
    })
}
