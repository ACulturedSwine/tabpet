...
function handleValidatePattern() {
    ...
    inputs.forEach((input)=> {
        ...
        input.addEventListener('input', function() {
            // for , soft invalid (prevent user from changing value + no warning)
            // for 
            if (this.checkValidity() || this.value === '') { // matches pattern AND maxlength (for number inputs since maxlength validation isn't built-in), or blank value
                prevVal = this.value;
            }
            else {
                this.value = prevVal;
