...
function handlePaging() {
    const pages = [
        ...
        new FormPage('done', '', '') // Last page has no title or description
    ];
...
function displayNewPage(pages, pageNum) {
    ...
    if (pageNum === pages.length) { // Show last page if form finished
        const formFinishedPageEl = document.getElementById('form-finished-page');
        formFinishedPageEl.style.display = 'block';
    }
    else { // Otherwise, show form page
        // Update instructions
        const pageInfo = pages[pageNum - 1];
        .textContent = pageInfo.title;
