...
function handlePhotoUpload() {
    ...
    window.addEventListener('focus', ()=> {
        console.log('focus');
        console.log(isFileDialogOpen, !uploadedFileInSession, !processingFile);
        if (isFileDialogOpen && !uploadedFileInSession && !processingFile) { // After closing window and not processing file
            isFileDialogOpen = false;
            uploadedFileInSession = false;
            const customPhotos = photoUpload.files;
            console.log(customPhotos);
            if (customPhotos[0]) { // Change to previous custom photo if exists
                photo.src = window.URL.createObjectURL(customPhotos[0]);
                selectPhoto.value = lastSelectValue = 'Custom';
            }
