...
class FormManager {
    ...
    */
    ...
    handleSaveInputs(currentPageEl) {
        ...
        questions.forEach(q=> { // Save if blank and valid (invalid answers are prevented in handleValidatePattern)
            ...
            // Save value on change and not blank
            q.addEventListener('input', function() {
                if (!formManager.#isBlank(q.value)) {

                }
                const isBlank = q.value.trim() === '';
                if (!isBlank) formManager.#saveInput(label, q.value.trim());
            })
        })
    }
