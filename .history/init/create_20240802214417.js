...
async function handlePaging() {
    await redirectIfPetInfo(); // Redirect to new tab if updated pet info
...
function handlePhotoUpload() {
    ...
    const preventFileUpload = true;

    selectPhoto.addEventListener('input', function() {
        if (this.value === 'Custom' && !preventFileUpload) { // TODO: Allow user to select same option 'Custom' twice and reupload
            photoUpload.click();
        }
        else {
