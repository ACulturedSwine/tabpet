...
async function saveAndRedirect() { // Save inputs to chrome storage, redirect to new tab
    ...
    chrome.storage.sync.set( { petInfo } ).then(async ()=> {
        let updatedPetInfo = await getPetInfo();
        if (updatedPetInfo) window.location.href = 'https://google.com';
        else throw new Error('Pet info not saved correctly');
    })
}

function redirectIfCompleted() { // Redirect to new tab if completed form and updated pet info correctly
