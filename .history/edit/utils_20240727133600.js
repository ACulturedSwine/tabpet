...
async function handleImgPaste(e, params) { // Does something if image given callbacks
    ...
    else if (!params || !params.callback && !params.fileCallback && !params.urlCallback) { // No callback defined
        console.error('Params not defined or needs callback', params);
    }

    let fileCallback = params.callback || params.fileCallback || null;
    let urlCallback = params.callback || params.urlCallback || null;

    if (fileCallback && res.type === 'file') {
        fileCallback(res.img);
    }
    else if (urlCallback && res.type === 'url') {
        urlCallback(res.img);
    }
}

export { isPastedImg, handleImgPaste };