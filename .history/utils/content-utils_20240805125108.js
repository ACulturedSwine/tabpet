...
function onMouseRelease(el, whileHold, onRelease) { // Function during mouse hold and function after when mouse release
    el.addEventListener('mousedown', ()=> {
        const whileHoldID = whileHold(); // whileHold must return setInterval id
        function handleMouseUp(e) {
            if (e.target === el) {
                onRelease(); // Execute function on mouse release
            }
            clearInterval(whileHoldID); // Stop function while mouse hold
            document.removeEventListener('mouseup', handleMouseUp); // Stop listening for mouseup
        }
...
function isFullyVisible(el) {
    return el.clientWidth < el.scrollWidth && el.clientHeight < el.scrollHeight; // Client width is visible content, scroll width is all content
}
...
export {
    ...
    onMouseRelease,
    isOverflowing, wait, isFullyVisible
};