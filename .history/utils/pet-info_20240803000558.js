...
function getBirthdayObject(petInfo) {
    return new Date(petInfo.year, petInfo.month - 1, petInfo.year);
}
...
function formatBirthday(petInfo) { // Return birthday with month typed out
    return new getBirthdayObject(petInfo).toDateString(); // Month is indexed at 0
}

function formatAge(petInfo) {
    var birthDate = new Date(petInfo.);
