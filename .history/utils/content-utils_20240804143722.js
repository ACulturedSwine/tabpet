...
function onMouseRelease(el, callbackOnHold, callbackOnRelease) { // Callback during mouse hold and callback after when mouse release
    el.addEventListener('mousedown', ()=> {
        const id = callbackOnHold(); // Callback on hold must return setInterval id
        function handleMouseUp(e) {
            if (e.target === el) {
                clearInterval(id); // Stop function on mouse hold
                callbackOnRelease(); // Execute function on mouse release
            }
