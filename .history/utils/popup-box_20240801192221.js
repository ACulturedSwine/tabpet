...
async function outOfBounds() {
    ...
    outOfBoundsTransition(popupTime); // Transition border color at the same time popup message starts to fade out
    await createPopupMsg({content: 'Out of bounds!', waitTime: popupTime});
}

async function outOfBoundsTransition(popupTime) { // During outOfBounds error, transition border color 
