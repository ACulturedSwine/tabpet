...
const maxParticles = 50; // Max particles on screen
const currentParticles = 0;
...
async function createParticles(type, position, parentContainer, n=1) {
    for (let i = 0; i < n && currentParticles; i++) {
        // 
        if (currentParticles + 1 > maxParticles) return;

        // Else, create particle and start float
