...
function pseudoTypeSearch() {
    ...
    const searchBarText = document.querySelector('YacQv').querySelector('span');
    if (searchBarTextArea === document.activeElement || searchBarText && searchBarText.textContent) return; // Prevent custom text if search bar text area is focused or contains text
