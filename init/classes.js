...
class FormManager {
    constructor() {
        ...
        this.setType('Gerbil'); // Set type of pet automatically to gerbil (only current one available)
    }
    ...
    */
    ...
    saveInput(questionEl) {
        ...
        this.saveInputManual(name, value, questionEl);
    }

    async saveInputManual(name, value, questionEl) {
        if (!value || value === '') return; // Don't save value if doesn't exist or blank

        console.log(value);
        
        if (questionEl && questionEl.type === 'file') {
